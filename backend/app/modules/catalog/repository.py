...
class CatalogRepository:
    ...
    async def list_categories(
        ...
        *,
        ...
        rows = await self.session.execute(
            query.order_by(Category.position, Category.name).limit(limit).offset(offset)
        )
    ...
    async def get_attribute(self, attribute_id: uuid.UUID) -> AttributeDefinition | None:
        return await self.session.get(AttributeDefinition, attribute_id)

    async def get_attribute_by_code(self, code: str) -> AttributeDefinition | None:
    ...
    async def list_attributes(
        ...
        *,
        ...
        rows = await self.session.execute(
            query.order_by(AttributeDefinition.name).limit(limit).offset(offset)
        )
        total = await self.session.scalar(count_query)
        return list(rows.scalars().all()), int(total or 0)
    ...
    async def link_all_global_attributes(self, category_id: uuid.UUID) -> None:
        result = await self.session.execute(
            ...
            .order_by(AttributeDefinition.created_at, AttributeDefinition.name)
        )
    ...
    async def list_category_attributes(
        ...
        result = await self.session.execute(
            ...
            .where(*filters)
            .order_by(CategoryAttribute.position, AttributeDefinition.name)
        )
