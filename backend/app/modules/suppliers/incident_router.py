...
@router.post(
    ...
    summary="Kreiraj ručni Incident",
    description="Kreira MANUAL operativni Incident bez izmene dobavljačkih ili Catalog podataka.",
)
async def create_manual(
    payload: ManualIncidentCreate, session: AsyncSession = Depends(get_db)
) -> IncidentRead:
    return IncidentRead.model_validate(
        await SupplierIncidentService(session).manual(payload)
    )
...
@router.get(
    ...
    summary="Prikaži Incidente",
    description="Ograničena pretraga bez source fajlova, tajni i kompletnog tehničkog konteksta.",
)
...
@router.get(
    ...
    summary="Prikaži Incident sažetak",
    description="Vraća aktivne statuse, prioritete, kašnjenje i raspodelu za budući dashboard.",
)
async def summary(
    supplier_id: uuid.UUID | None = None, session: AsyncSession = Depends(get_db)
) -> SummaryRead:
    return SummaryRead.model_validate(
        await SupplierIncidentQueryService(session).summary(supplier_id)
    )
...
@router.get(
    "/{incident_id}",
    response_model=IncidentRead,
    summary="Prikaži Incident",
    description="Incident je operativni workflow; izvorni Acquisition, Snapshot i Delta ostaju nepromenjeni.",
)
async def get_incident(
    incident_id: uuid.UUID, session: AsyncSession = Depends(get_db)
) -> IncidentRead:
    return IncidentRead.model_validate(
        await SupplierIncidentQueryService(session).get(incident_id)
    )
...
@router.get(
    ...
    response_model=IncidentRead,
    summary="Prikaži reference dokaza",
    description="Vraća bezbedne reference, bez source payload-a, stack trace-a ili tajni.",
)
async def evidence(
    incident_id: uuid.UUID, session: AsyncSession = Depends(get_db)
) -> IncidentRead:
...
@router.get(
    ...
    summary="Prikaži audit događaje",
    description="Immutable istorija pojave, statusa, dodele, komentara i korelacije.",
)
async def events(
    ...
    rows, total = await service.repository.events(
...
@router.get(
    ...
    summary="Prikaži komentare",
    description="Paginirani bezbedni plain-text komentari.",
)
async def comments(
    incident_id: uuid.UUID,
    limit: int = Query(100, ge=1, le=500),
    ...
    service = SupplierIncidentQueryService(session)
    await service.get(incident_id)
    rows, total = await service.repository.comments(
        incident_id, limit=limit, offset=offset
    )
    return CommentList(
        items=[CommentRead.model_validate(row) for row in rows], total=total
    )
...
@router.post(
    ...
    description="Dodaje sanitizovan komentar i atomarni COMMENT_ADDED audit događaj.",
)
async def add_comment(
    ...
    return CommentRead.model_validate(
        await SupplierIncidentService(session).comment(incident_id, payload.body)
    )
