...
class JsonParser:
    def parse(
        ...
        if isinstance(value, dict):
            for key in ("items", "products", "records", "data", "catalog"):
...
class XlsxParser:
    ...
    def parse(
        ...
        try:
            with zipfile.ZipFile(io.BytesIO(content)) as archive:
                ...
                zero_widths = self._zero_padded_widths(archive)
                sheet_path = self._sheet_path(archive, configuration)
                rows = self._rows_with_formats(
                    archive.read(sheet_path), shared, zero_widths
                )
        except (
            KeyError,
            ValueError,
    ...
    def _rows(cls, content: bytes, shared: list[str]) -> dict[int, list[object]]:
        return cls._rows_with_formats(content, shared, {})

    @classmethod
    def _rows_with_formats(
        cls,
        content: bytes,
        shared: list[str],
        zero_widths: dict[int, int],
        ...
        for row in (node for node in root.iter() if node.tag.endswith("}row")):
            ...
            for cell in (node for node in row if node.tag.endswith("}c")):
                ...
                if width and cell.attrib.get("t") not in {"s", "str"}:
                    rendered = rendered.zfill(width)
                values[index] = rendered
            rows[number] = values
        return rows

    @staticmethod
    def _zero_padded_widths(archive: zipfile.ZipFile) -> dict[int, int]:
        try:
        ...
        formats = {
            ...
            if node.tag.endswith("}numFmt") and "numFmtId" in node.attrib
        }
        cell_xfs = next(
            (node for node in root.iter() if node.tag.endswith("}cellXfs")),
            None,
        )
        if cell_xfs is None:
            return {}
        ...
        for index, style in enumerate(cell_xfs):
            code = formats.get(int(style.attrib.get("numFmtId", "0")), "")
