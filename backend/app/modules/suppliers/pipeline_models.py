...
class SupplierSourceSchedule(UUIDMixin, TimestampMixin, Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            ...
            name="uq_supplier_source_schedules_source_connection_id",
        ),
        Index(
            "ix_supplier_source_schedules_due",
            "status",
            "next_run_at",
        ...
        CheckConstraint(
            "schedule_type IS NULL OR schedule_type IN ('DAILY','MULTI_DAILY','INTERVAL','WEEKDAYS','WEEKLY')",
            name="schedule_type_valid",
        ),
        CheckConstraint(
            "automation_depth IN ('FETCH_ONLY','FETCH_AND_ANALYZE','FULL_PIPELINE')",
            name="automation_depth_valid",
        ),
    ...
    timezone: Mapped[str] = mapped_column(
        String(100), nullable=False, default="Europe/Belgrade"
    )
    schedule_configuration: Mapped[dict[str, object]] = mapped_column(
        JSONB, nullable=False, default=dict
    )
    automation_depth: Mapped[str] = mapped_column(
        String(32), nullable=False, default="FETCH_ONLY"
    )
...
class SupplierSourceArtifact(UUIDMixin, TimestampMixin, Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "artifact_code", name="uq_supplier_source_artifacts_artifact_code"
        ),
        UniqueConstraint(
            "storage_reference", name="uq_supplier_source_artifacts_storage_reference"
        ),
        Index(
            "ix_supplier_source_artifacts_source_created",
            "source_connection_id",
            "created_at",
            "id",
        ),
        Index(
            "ix_supplier_source_artifacts_source_checksum",
            "source_connection_id",
            "checksum_sha256",
        ),
        ...
        CheckConstraint(
            "detected_format IN ('CSV','XLSX','XML','JSON')", name="format_valid"
        ),
        CheckConstraint(
            "retention_status IN ('ONLINE','ARCHIVED')", name="retention_status_valid"
        ),
    )
    source_connection_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("supplier_sources.id", ondelete="RESTRICT"), nullable=False
    ...
    artifact_code: Mapped[str] = mapped_column(
        ...
        nullable=False,
        server_default=text(
            "'ART-' || lpad(nextval('supplier_source_artifact_code_seq'::regclass)::text, 8, '0')"
        ),
    )
...
class SupplierSourcePipelineRun(UUIDMixin, TimestampMixin, Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "pipeline_code", name="uq_supplier_source_pipeline_runs_pipeline_code"
        ),
        UniqueConstraint(
            "idempotency_key", name="uq_supplier_source_pipeline_runs_idempotency_key"
        ),
        ...
        Index(
            "ix_supplier_source_pipeline_runs_source_created",
            "source_connection_id",
            "created_at",
            "id",
        ),
        Index(
            "ix_supplier_source_pipeline_runs_schedule_occurrence",
            "schedule_id",
        ...
        CheckConstraint(
            "trigger_type IN ('MANUAL','SCHEDULED')", name="trigger_type_valid"
        ),
        CheckConstraint(
            "automation_depth IN ('FETCH_ONLY','FETCH_AND_ANALYZE','FULL_PIPELINE')",
            name="automation_depth_valid",
    ...
    schedule_id: Mapped[uuid.UUID | None] = mapped_column(
        ForeignKey("supplier_source_schedules.id", ondelete="SET NULL")
    )
    ...
    pipeline_code: Mapped[str] = mapped_column(
        ...
        nullable=False,
        server_default=text(
            "'PIP-' || lpad(nextval('supplier_source_pipeline_code_seq'::regclass)::text, 8, '0')"
        ),
    )
    ...
    current_phase: Mapped[str] = mapped_column(
        String(32), nullable=False, default="CURRENCY_RATE"
    )
    phase_results: Mapped[dict[str, object]] = mapped_column(
        JSONB, nullable=False, default=dict
    )
    artifact_id: Mapped[uuid.UUID | None] = mapped_column(
        ForeignKey("supplier_source_artifacts.id", ondelete="RESTRICT")
    )
    active_schema_profile_id: Mapped[uuid.UUID | None] = mapped_column(
        ForeignKey("supplier_schema_profiles.id", ondelete="RESTRICT")
    )
    analyzed_schema_profile_id: Mapped[uuid.UUID | None] = mapped_column(
        ForeignKey("supplier_schema_profiles.id", ondelete="RESTRICT")
    )
    active_mapping_profile_id: Mapped[uuid.UUID | None] = mapped_column(
    ...
    schedule_occurrence_at: Mapped[datetime | None] = mapped_column(
    ...
    version: Mapped[int] = mapped_column(Integer, nullable=False, default=1)
    __mapper_args__ = {"version_id_col": version, "version_id_generator": False}
    compatibility_report: Mapped[SupplierSchemaCompatibilityReport | None] = (
        relationship(back_populates="pipeline_run", uselist=False)
    )
...
class SupplierSchemaCompatibilityReport(UUIDMixin, Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "pipeline_run_id",
            name="uq_supplier_schema_compatibility_reports_pipeline_run_id",
        ),
        Index(
            "ix_supplier_schema_compatibility_reports_active_schema_created",
            "active_schema_profile_id",
            "created_at",
        ),
        CheckConstraint(
            "result IN ('COMPATIBLE','COMPATIBLE_WITH_WARNINGS','INCOMPATIBLE')",
            name="result_valid",
        ),
        CheckConstraint(
            "severity IN ('INFO','WARNING','ERROR','CRITICAL')", name="severity_valid"
        ),
    )
    ...
    active_schema_profile_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("supplier_schema_profiles.id", ondelete="RESTRICT"), nullable=False
    )
    analyzed_schema_profile_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("supplier_schema_profiles.id", ondelete="RESTRICT"), nullable=False
    )
    ...
    version: Mapped[int] = mapped_column(Integer, nullable=False, default=1)
    pipeline_run: Mapped[SupplierSourcePipelineRun] = relationship(
        back_populates="compatibility_report"
    )
