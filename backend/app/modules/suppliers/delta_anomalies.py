...
def anomaly_signals(
    *,
    ...
    if (
        previous_total >= minimum_items
        and removed / previous_total >= high_removal_ratio
    ):
        signals.append(
            {"code": "HIGH_REMOVAL_RATIO", "ratio": removed / previous_total}
        )
    ...
    if (
        previous_total >= minimum_items
        and modified / previous_total >= unusual_modified_ratio
    ):
        signals.append(
            {"code": "UNUSUAL_FIELD_CHANGE_VOLUME", "ratio": modified / previous_total}
        )
    return signals
