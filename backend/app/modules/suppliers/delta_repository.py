...
class SupplierDeltaRepository:
    ...
    async def previous_ready(
        self, current: SupplierSnapshot
        ...
        return (
            await self.session.execute(
                ...
                .where(
                    ...
                    SupplierSnapshot.source_connection_id
                    == current.source_connection_id,
                    SupplierSnapshot.status == "READY",
                    SupplierSnapshot.created_at < current.created_at,
                )
                .order_by(
                    SupplierSnapshot.created_at.desc(), SupplierSnapshot.id.desc()
                )
                .limit(1)
            )
        ).scalar_one_or_none()
    ...
    async def mutate_run(
        self, run: SupplierDeltaRun, changes: dict[str, object]
    ) -> None:
    ...
    async def list_runs(
        self, supplier_id: uuid.UUID, source_id: uuid.UUID, *, limit: int, offset: int
    ) -> tuple[list[SupplierDeltaRun], int]:
        filters = [
            SupplierDeltaRun.supplier_id == supplier_id,
            SupplierDeltaRun.source_connection_id == source_id,
        ]
        total = await self.session.scalar(
            select(func.count(SupplierDeltaRun.id)).where(*filters)
        )
        rows = await self.session.execute(
            select(SupplierDeltaRun)
            .where(*filters)
            .order_by(SupplierDeltaRun.created_at.desc(), SupplierDeltaRun.id.desc())
            .limit(limit)
            .offset(offset)
        )
    ...
    async def list_delta_items(
        ...
        *,
        ...
        for column, value in (
            (SupplierDeltaItem.change_type, change_type),
            (SupplierDeltaItem.has_price_change, price),
            (SupplierDeltaItem.has_stock_change, stock),
            (SupplierDeltaItem.has_image_change, image),
            (SupplierDeltaItem.has_identifier_change, identifier),
        ):
        ...
        total = await self.session.scalar(
            select(func.count(SupplierDeltaItem.id)).where(*filters)
        )
        rows = await self.session.execute(
            select(SupplierDeltaItem)
            .where(*filters)
            .order_by(SupplierDeltaItem.created_at, SupplierDeltaItem.id)
            .limit(limit)
            .offset(offset)
        )
        return list(rows.scalars()), int(total or 0)

    async def get_delta_item(
        self, run_id: uuid.UUID, item_id: uuid.UUID
    ) -> SupplierDeltaItem | None:
        return (
            await self.session.execute(
                select(SupplierDeltaItem).where(
                    SupplierDeltaItem.id == item_id,
                    SupplierDeltaItem.delta_run_id == run_id,
                )
            )
        ).scalar_one_or_none()

    async def field_changes(
        self, item_id: uuid.UUID, *, limit: int, offset: int
    ) -> tuple[list[SupplierDeltaFieldChange], int]:
        total = await self.session.scalar(
            select(func.count(SupplierDeltaFieldChange.id)).where(
                SupplierDeltaFieldChange.delta_item_id == item_id
            )
        )
        rows = await self.session.execute(
            select(SupplierDeltaFieldChange)
            .where(SupplierDeltaFieldChange.delta_item_id == item_id)
            .order_by(SupplierDeltaFieldChange.field_path)
            .limit(limit)
            .offset(offset)
        )
        return list(rows.scalars()), int(total or 0)
