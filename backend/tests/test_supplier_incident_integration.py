...
async def _facts(
    supplier_id: str, source_id: str, snapshots: list[str]
    ...
    async with sessions() as session:
        template = (
            await session.execute(
                ...
                .where(SupplierAcquisitionRun.supplier_id == uuid.UUID(supplier_id))
                .limit(1)
            )
        ).scalar_one()
        failed = SupplierAcquisitionRun(
            supplier_id=uuid.UUID(supplier_id),
            source_connection_id=uuid.UUID(source_id),
            schema_profile_id=template.schema_profile_id,
            mapping_profile_id=template.mapping_profile_id,
        ...
        assert (
            previous
            and current
            and previous.snapshot_fingerprint
        ...
        delta = SupplierDeltaRun(
            ...
            total_previous_items=3,
            total_current_items=1,
            removed_items=2,
            modified_items=1,
            price_increased_items=1,
            image_changed_items=1,
            anomaly_signals=signals,
            completed_at=current.finalized_at,
            created_by="incident-test",
        )
        session.add(delta)
    ...
    async with sessions() as session:
        incident_ids = list(
            await session.scalars(
                select(SupplierIncident.id).where(
                    SupplierIncident.supplier_id == identifier
                )
            )
        )
        if incident_ids:
            await session.execute(
                delete(SupplierIncidentLink).where(
                    (SupplierIncidentLink.incident_id.in_(incident_ids))
                    | (SupplierIncidentLink.related_incident_id.in_(incident_ids))
                )
            )
            await session.execute(
                delete(SupplierIncidentComment).where(
                    SupplierIncidentComment.incident_id.in_(incident_ids)
                )
            )
            await session.execute(
                delete(SupplierIncidentEvent).where(
                    SupplierIncidentEvent.incident_id.in_(incident_ids)
                )
            )
            await session.execute(
                delete(SupplierIncident).where(SupplierIncident.id.in_(incident_ids))
            )
        await session.execute(
            delete(SupplierIncidentRule).where(
                SupplierIncidentRule.supplier_id == identifier
            )
        )
        await session.commit()
        ...
        try:
            ...
            for index in range(2):
                run = client.post(
                    ...
                    params={"filename": f"incident-{index}.csv"},
                    headers={"Content-Type": "text/csv"},
                    content=_csv_payload("dugačak opis " * 2000),
                )
                snapshot = client.post(
                    f"{root}/snapshots", json={"acquisition_run_id": run.json()["id"]}
                )
                snapshots.append(snapshot.json()["id"])
            failed_id, delta_id, original_signals = asyncio.run(
                _facts(supplier_id, source_id, snapshots)
            )
            products_before = client.get("/products").json()["total"]
            inventory_before = client.get("/inventory").json()["total"]
            rule = client.post(
                "/supplier-incident-rules",
                json={
                    "rule_code": f"IMAGE_{suffix.upper()}",
                    "name": "Image anomaly",
                    "source_domain": "DELTA",
                    "incident_type": "IMAGE_SET_CHANGED",
                    "signal_code": "IMAGE_SET_CHANGED",
            ...
            acquisition = client.post(
                f"/supplier-incidents/sync/acquisition-runs/{failed_id}"
            )
            ...
            removal = next(
                row
                for row in repeated.json()
                if row["incident_type"] == "HIGH_REMOVAL_RATIO"
            )
            acknowledged = client.post(
                f"/supplier-incidents/{removal['id']}/acknowledge"
            )
            assert acknowledged.json()["status"] == "ACKNOWLEDGED"
            assigned = client.post(
                f"/supplier-incidents/{removal['id']}/assign",
                json={"assigned_user_id": "operator-1"},
            )
            assert assigned.json()["assigned_user_id"] == "operator-1"
            comment = client.post(
                f"/supplier-incidents/{removal['id']}/comments",
                json={"body": "<script>alert(1)</script> Provera"},
            )
            assert "<script>" not in comment.json()["body"]
            assert (
                client.post(f"/supplier-incidents/{removal['id']}/start").json()[
                    "status"
                ]
                == "IN_PROGRESS"
            )
            resolved = client.post(
                f"/supplier-incidents/{removal['id']}/resolve",
                json={
                    "resolution_code": "DATA_CONFIRMED",
                    "resolution_summary": "Provereno",
                },
            )
            assert resolved.json()["status"] == "RESOLVED"
            recurrence = client.post(
                f"/supplier-incidents/sync/delta-runs/{delta_id}"
            ).json()
            reopened = next(row for row in recurrence if row["id"] == removal["id"])
            assert reopened["status"] == "OPEN" and reopened["occurrence_count"] == 3
            image = next(
                row for row in recurrence if row["incident_type"] == "IMAGE_SET_CHANGED"
            )
            suppressed = client.post(
                f"/supplier-incidents/{image['id']}/suppress",
                json={"reason": "Prihvaćena buka"},
            )
            assert suppressed.json()["status"] == "SUPPRESSED"
            suppressed_recurrence = client.post(
                f"/supplier-incidents/sync/delta-runs/{delta_id}"
            ).json()
            image_again = next(
                row for row in suppressed_recurrence if row["id"] == image["id"]
            )
            assert (
                image_again["status"] == "SUPPRESSED"
                and image_again["occurrence_count"] == 4
            )
            ...
            manual = client.post(
                ...
                json={
                    "supplier_id": supplier_id,
                    "source_connection_id": source_id,
                    "incident_type": "MANUAL_DATA_QUALITY_REPORT",
                    "severity": "MEDIUM",
                    "priority": "P3",
                    "title": "Ručna provera",
                    "description": "Opis",
            ...
            assert (
                client.get(f"{root}/deltas/{delta_id}").json()["anomaly_signals"]
                == original_signals
            )
            assert all(
                client.get(f"{root}/snapshots/{snapshot_id}").json()["status"]
                == "READY"
