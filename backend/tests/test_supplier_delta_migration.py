    ...
    try:
        tables = {
            ...
            for row in await connection.fetch(
                "SELECT tablename FROM pg_tables WHERE schemaname='public' AND tablename LIKE 'supplier_delta%'"
            )
        }
        sequence = bool(
            await connection.fetchval(
                "SELECT to_regclass('public.supplier_delta_code_seq') IS NOT NULL"
            )
        )
