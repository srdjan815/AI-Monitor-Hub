...
def test_incident_layers_and_frozen_boundaries() -> None:
    ...
    helpers = (SUPPLIERS / "incident_safety.py").read_text(encoding="utf-8") + (
        SUPPLIERS / "incident_rules.py"
    ).read_text(encoding="utf-8")
    assert (
        "AsyncSession" not in helpers
        and "eval(" not in helpers
        and "exec(" not in helpers
    )
...
def test_incident_migration_is_additive() -> None:
    ...
    assert {
        SupplierIncident.__tablename__,
        SupplierIncidentEvent.__tablename__,
        SupplierIncidentComment.__tablename__,
        SupplierIncidentLink.__tablename__,
        SupplierIncidentRule.__tablename__,
    } == {
        "supplier_incidents",
...
def test_incident_openapi_and_permissions() -> None:
    ...
    assert ROLE_PERMISSIONS["read_only"].intersection(required) == {
        "incidents.read",
        "incident_rules.read",
    }
