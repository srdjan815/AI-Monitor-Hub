    ...
    async with sessions() as session:
        ...
        items = list(
            await session.scalars(
                ...
                .where(SupplierSnapshotItem.snapshot_id == snapshot.id)
        ...
        for position, item in enumerate(items, 1):
            ...
            data["price"] = {
                "SKU-001": "55.00" if current else "50.00",
                "SKU-002": "100.00",
                "SKU-003": "200.00",
                "SKU-004": "300.00",
            }[code]
            data["stock"] = {
                "SKU-001": 0 if current else 10,
                "SKU-002": 5,
                "SKU-003": 1,
                "SKU-004": 4,
            }[code]
            if code == "SKU-001":
                ...
                item.source_image_links = [
                    ...
                    {
                        "url": f"https://img.test/{'image-c' if current else 'image-b'}.jpg",
                        "role": "GALLERY",
                    },
                ]
            item.mapped_data = data
            item.item_fingerprint = item_fingerprint(
                data, item.source_image_links, item.source_key, item.source_identifier
            )
        ...
        try:
            ...
            for index in range(2):
                ...
                snapshot = client.post(
                    snapshots_root, json={"acquisition_run_id": run.json()["id"]}
                )
            ...
            assert (
                delta["added_items"],
                delta["removed_items"],
                delta["modified_items"],
                delta["unchanged_items"],
            ) == (1, 1, 1, 1)
            ...
            changed = client.get(
                f"{endpoint}/{delta['id']}/items", params={"change_type": "MODIFIED"}
            ).json()["items"][0]
            fields = client.get(
                f"{endpoint}/{delta['id']}/items/{changed['id']}/field-changes"
            ).json()["items"]
            description = next(
                field for field in fields if field["field_path"] == "description"
            )
            assert (
                description["previous_value_hash"] != description["current_value_hash"]
            )
            assert len(description["previous_value_preview"]) <= 240
            assert long_text not in str(fields)
            reviews = client.get(
                "/suppliers/platform/article-reviews",
            ...
            offloaded = client.post(
                f"{snapshots_root}/{snapshot_ids[0]}/offload",
                json={
                    "operation_id": exported["id"],
                    "archive_reference": exported["archive_reference"],
                    "archive_checksum": exported["archive_checksum"],
                },
            )
            assert offloaded.status_code == 200
            archived = client.get(
                f"{endpoint}/compatibility",
                params={
                    "previous_snapshot_id": snapshot_ids[0],
                    "current_snapshot_id": snapshot_ids[1],
                },
