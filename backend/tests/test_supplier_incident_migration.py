    ...
    try:
        tables = {
            ...
            for row in await connection.fetch(
                "SELECT tablename FROM pg_tables WHERE schemaname='public' AND tablename LIKE 'supplier_incident%'"
            )
        }
        sequence = bool(
            await connection.fetchval(
                "SELECT to_regclass('public.supplier_incident_code_seq') IS NOT NULL"
            )
        )
