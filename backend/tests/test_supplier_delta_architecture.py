...
def test_delta_migration_and_models_are_additive() -> None:
    ...
    assert {
        SupplierDeltaRun.__tablename__,
        SupplierDeltaItem.__tablename__,
        SupplierDeltaFieldChange.__tablename__,
    } == {
        "supplier_delta_runs",
...
def test_delta_openapi_and_permissions() -> None:
    paths = {
        path: ops for path, ops in app.openapi()["paths"].items() if "/deltas" in path
    }
    assert len(paths) == 10
    assert all(
        op.get("summary") and op.get("description")
