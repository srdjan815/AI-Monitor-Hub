...
async def list_categories(
    ...
    return CategoryList(
        items=[CategoryRead.model_validate(row) for row in rows], total=total
    )
...
async def deactivate_category(
    ...
    await CatalogService(session).update_category(
        category_id, CategoryUpdate(is_active=False)
    )
...
async def create_attribute(
    payload: AttributeCreate, session: AsyncSession = Depends(get_db)
) -> AttributeRead:
...
async def list_attributes(
    ...
    rows, total = await CatalogRepository(session).list_attributes(
        scope=scope, active_only=active_only, limit=limit, offset=offset
    )
    return AttributeList(
        items=[AttributeRead.model_validate(row) for row in rows], total=total
    )
...
async def update_attribute(
    ...
    attribute = await CatalogService(session).update_attribute(attribute_id, payload)
    return AttributeRead.model_validate(attribute)


@router.get(
    "/categories/{category_id}/attributes", response_model=list[CategoryAttributeRead]
)
...
async def reorder_category_attributes(
    ...
    await CatalogService(session).reorder_category_attributes(category_id, payload)
    return Response(status_code=status.HTTP_204_NO_CONTENT)
