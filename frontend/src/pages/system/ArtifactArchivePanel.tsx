...
export function ArtifactArchivePanel() {
  ...
  return <Paper sx={{ p: 2.5, mt: 3 }}>
    ...
    <Stack direction="row" gap={2} my={2} flexWrap="wrap" alignItems="flex-start">
      <Stack gap={0.5} sx={{ flex: "1 1 300px", minWidth: 260 }}><FieldInfoLabel label="Naziv odredišta" help="Prepoznatljiv naziv lokalnog ili NAS odredišta. Ne utiče na fizičku putanju niti sadrži pristupne podatke." /><TextField aria-label="Naziv odredišta" value={name} onChange={(event) => setName(event.target.value)} /></Stack>
      <Stack gap={0.5} sx={{ flex: "1 1 340px", minWidth: 300 }}><FieldInfoLabel label="Podfolder originalnih cenovnika" help="Relativni podfolder unutar centralno montiranog arhivskog odredišta u koji se kopiraju originalni cenovnici." /><TextField aria-label="Podfolder originalnih cenovnika" value={path} onChange={(event) => setPath(event.target.value)} error={Boolean(pathError)} helperText={pathError ?? "Primer: cenovnici ili cenovnici/2026"} /></Stack>
      <Stack gap={0.5} sx={{ flex: "1 1 300px", minWidth: 280 }}><FieldInfoLabel label="Podfolder snapshot arhive" help="Odvojeni relativni podfolder za verifikovane snapshot ZIP arhive potrebne za kasniju obnovu podataka." /><TextField aria-label="Podfolder snapshot arhive" value={snapshotPath} onChange={(event) => setSnapshotPath(event.target.value)} error={Boolean(snapshotPathError)} helperText={snapshotPathError ?? "Primer: snapshots"} /></Stack>
      <Stack gap={0.5} sx={{ flex: "0 1 260px", minWidth: 240 }}><FieldInfoLabel label="Lokalno zadržavanje (dana)" help="Koliko dana original ostaje u lokalnom brzom skladištu. Brisanje nije dozvoljeno dok udaljena kopija nije uspešno verifikovana." /><TextField aria-label="Lokalno zadržavanje (dana)" type="number" value={days} onChange={(event) => setDays(Number(event.target.value))} inputProps={{ min: 1, max: 3650 }} /></Stack>
      <Stack gap={0.5} sx={{ minWidth: 210 }}><FieldInfoLabel label="Automatski prenos" help="Kada je uključeno, sistem prenosi cenovnike u serijama samo na prethodno uspešno testirano odredište." /><FormControlLabel sx={{ minHeight: 56, m: 0 }} control={<Switch checked={enabled} onChange={(event) => setEnabled(event.target.checked)} />} label={enabled ? "Uključen" : "Isključen"} /></Stack>
    </Stack>
