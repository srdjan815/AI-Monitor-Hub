...
export function ArtifactArchivePanel() {
  ...
  const save = useMutation({ mutationFn: () => api("/system/resources/artifact-archive/setting", { method: "PUT", body: { display_name: name, relative_path: path, enabled, local_retention_days: days, expected_version: status.data?.setting?.version ?? null } }), onSuccess: refresh });
  ...
  return <Paper sx={{ p: 2.5, mt: 3 }}>
    ...
    <Stack direction={{ xs: "column", md: "row" }} gap={2} my={2}><TextField label="Naziv odredišta" value={name} onChange={(event) => setName(event.target.value)} fullWidth /><TextField label="Relativna putanja unutar mount-a" value={path} onChange={(event) => setPath(event.target.value)} fullWidth helperText="Bez lozinki i apsolutne putanje" /><TextField label="Lokalno zadržavanje (dana)" type="number" value={days} onChange={(event) => setDays(Number(event.target.value))} inputProps={{ min: 1, max: 3650 }} /><FormControlLabel control={<Switch checked={enabled} onChange={(event) => setEnabled(event.target.checked)} />} label="Automatski prenos" /></Stack>
    <Stack direction="row" gap={1} flexWrap="wrap"><Button variant="contained" onClick={() => save.mutate()} disabled={save.isPending}>Sačuvaj</Button><Button onClick={() => test.mutate()} disabled={!data?.setting || test.isPending}>Testiraj odredište</Button><Button onClick={() => process.mutate()} disabled={!data?.setting?.enabled || process.isPending}>Obradi sledećih 25</Button></Stack>
    ...
    {operationError && <Alert severity="error" sx={{ mt: 1 }}>{operationError instanceof Error ? operationError.message : "Operacija nije uspela."}</Alert>}
